...
public class Customerprofilewnd extends Div{
	...
	/*
	 * Label lb = (Label)this.getFellow("");
	 * 
	 * if (lt == null) return; Customerprofile c= (Customerprofile)lt.getValue();
	 * 
	 * Map<String, Customerprofile> params = new HashMap<String,Customerprofile>();
	 * params.put("Customerprofile", c);
	 */
		Window win = (Window)Executions.createComponents("Customerprofileedit.zul", null, null);
		win.doModal();
	/*
	 * if (win.getAttribute("OK") != null) { //stupid findc =
	 * CustomerprofileDAO.Findcustomer();
	 */
